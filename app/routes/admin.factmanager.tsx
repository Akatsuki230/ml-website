...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  const facts = await (await fetch(`${process.env.FIREBASE_URL}/facts.json`)).json()
  ...
  return (
    <>
      ...
      <form action='/admin/factmanager' method='POST' className='m-1'>
        ...
        <input type='text' name='fact' id='fact' className='block m-1 bg-black' />
        <button type='submit' className='bg-blue-600 p-1 px-2 m-1 rounded-md'>Add</button>
      </form>

      <label htmlFor='search'>Search</label>
      <input type='text' value={search} onChange={x => setSearch(x.currentTarget.value)} className='bg-black' id='search'/>
      ...
      <table>
        ...
        <tbody>
          {data.map((fact: { fact: string}, i: number) => {
            if (!fact.fact.toLowerCase().includes(search.toLowerCase())) return null
            ...
            return <tr key={i}>
              <td>{fact.fact}</td>
              <td>
                <button className='bg-red-600 p-1 px-2 m-1 rounded-md' onClick={deleteFact}>Delete</button>
              </td>
            </tr>
          })}
        </tbody>
