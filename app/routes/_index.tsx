...
export default function Index() {
  ...
  return (
    <>
      <h1 className='text-4xl mx-2 my-4 text-center'>Welcome to mldkyt's website! <span className='text-yellow-400 text-sm'>Now rewritten in Remix!</span></h1>
      ...
      <div className='mx-1'>mldkyt has experience in {showProgrammingList ?
        <span>C#, C++, C, Python, Java, Kotlin, JavaScript, TypeScript,
                    HTML, CSS and more. <span className='text-blue-400 cursor-pointer'
                                              onClick={() => setShowProgrammingList(false)}>hide</span></span> :
        <span className='text-blue-400 cursor-pointer'
              onClick={() => setShowProgrammingList(true)}>show languages</span>}
      </div>
      ...
      <div className='mx-1'>You can join my <a href='https://discord.gg/JgFNmSwYME'
                                               className='text-blue-400'>Discord</a> server. You can also subscribe
        on <a href='https://youtube.com/@mldkyt'
      ...
      <div className='flex'>
        <a href='https://youtube.com/@mldkyt'>
          <motion.button
            initial={{scale: 1.0, backgroundImage: 'linear-gradient(to bottom, #511, #511, #111)'}}
            whileHover={{scale: 1.1, backgroundImage: 'linear-gradient(to bottom, #511, #511, #211)'}}
            whileTap={{scale: 0.9, backgroundImage: 'linear-gradient(to bottom, #311, #111, #111)'}}
            className='px-8 py-4 m-2 bg-gradient-to-b rounded-xl'
          >
            YouTube
          </motion.button>
        </a>
        <a href='https://tiktok.com/@mldkyt'>
          <motion.button
            initial={{scale: 1.0, backgroundImage: 'linear-gradient(to bottom, #155, #155, #111)'}}
            whileHover={{scale: 1.1, backgroundImage: 'linear-gradient(to bottom, #155, #155, #122)'}}
            whileTap={{scale: 0.9, backgroundImage: 'linear-gradient(to bottom, #133, #111, #111)'}}
            className='px-8 py-4 m-2 bg-gradient-to-b rounded-xl'
          >
            TikTok
          </motion.button>
        ...
        <a href='/discord'>
          <motion.button
            initial={{scale: 1.0, backgroundImage: 'linear-gradient(to bottom, #136, #136, #111)'}}
            whileHover={{scale: 1.1, backgroundImage: 'linear-gradient(to bottom, #136, #136, #122)'}}
            whileTap={{scale: 0.9, backgroundImage: 'linear-gradient(to bottom, #123, #111, #111)'}}
            className='px-8 py-4 m-2 bg-gradient-to-b rounded-xl'
          >
            Discord Server
          </motion.button>
      ...
      <span className='text-blue-500 cursor-pointer'
            onClick={() => navigator.clipboard.writeText('https://mldkyt.com/newestvideo')}>
                    <motion.button
                      whileHover={{scale: 1.1}}
                      whileTap={{scale: 0.9}}>
                        (Copy link)
      ...
      <span className='text-blue-500 cursor-pointer'
            onClick={() => navigator.clipboard.writeText('https://mldkyt.com/neweststream')}>
                    <motion.button
                      whileHover={{scale: 1.1}}
                      whileTap={{scale: 0.9}}>
                        (Copy link)
      ...
      <div className='bg-[#2B0029] rounded-lg p-2 mx-4'>
        ...
        <p className='ml-4'>The meshsave is a save for My Summer Car that makes the car look like it's been
          through world wars.</p>
        <a href='/meshsave'>
          <motion.button
            whileHover={{scale: 1.1}}
            whileTap={{scale: 0.9}}
      ...
      <div className='bg-[#2B0029] rounded-lg p-2 mx-4 mt-2'>
        ...
        <p className='ml-4'>This is my social link list. You will find more than what you just saw
          above.</p>
        <a href='/social'>
          <motion.button
            whileHover={{scale: 1.1}}
      ...
      <p className='absolute bottom-2 left-2 italic'>p.s. i am not a webpage designer and i am not native
        english</p>
      <Navbar sel='home'/>
    </>
  );
