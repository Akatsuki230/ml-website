...
export default function DiscordBanned() {
  ...
  const [appealErrorMessage, setAppealErrorMessage] = useState(
    "" as string | null
  );
  ...
  const checkReason = async () => {
    ...
    try {
      const response = await fetch(
        '/api/discord/ban?username=${encodeURIComponent(userName)}'
      );
      if (!response.ok) {
      ...
      <Container>
        ...
        {showInitialUI && (
          <>
            ...
            <p>
              Here you can check the ban reason if you were banned from
        ...
        {showResultUI && (
          <>
            ...
            {showAppealMessage && (
              <>
                ...
                <Form.Control
                  as="textarea"
                  value={appealMessage}
                  onChange={(x) => setAppealMessage(x.target.value)}
                />
                <br />
                <Button onClick={sendAppeal}>Send appeal</Button>
              </>
            )}
            {appealMessageSending && (
              <>
                <Spinner animation="border" role="status" />
              </>
            )}
            ...
            {appealMessageError && (
              <>
                <p>Error sending appeal: {appealErrorMessage}</p>
              </>
            )}
        ...
        {showErrorUI && (
          <>
            <h1>Error</h1>
            <p>
              An error has occured, check your internet connection and try
              again.
            </p>
